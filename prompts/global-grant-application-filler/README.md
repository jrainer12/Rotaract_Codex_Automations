...
Copy everything in the prompt below and paste it into an AI tool that can read and create files. Upload the blank Rotary Global Grant application template when the AI asks for it.

See `sample-interaction.txt` for a made-up example run. See `sample-files/` for the blank template files used with this prompt.
...
2. Read the uploaded template and identify every question, table, checkbox, and document-upload instruction.
...
5. If a question does not apply to my grant type, mark it as "Not applicable" and briefly explain why in the draft.
6. Keep answers concise, specific, and appropriate for a Rotary Foundation Global Grant application.
7. When enough information has been collected, create a completed DOCX file using the same structure as the template. Preserve headings, section order, tables, checkbox labels, and any planning notes that are part of the template.
...
1. A completed DOCX application file for download.
2. A brief completion summary.
3. A checklist of missing attachments and fields still requiring confirmation.
4. A list of assumptions you made while filling the application.
